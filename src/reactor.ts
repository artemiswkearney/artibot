...
type Handler = [EmoteID, Callback, boolean]; // [2]: display the reaction under the message
type Filter = (m : Discord.Message) => Promise<boolean>;
type FilteredHandler = [EmoteID, Filter, Callback, boolean]; // [2]: display the reaction under the message
...
async function addHandlers(channel : ChannelID, handlers : Handler[]) : Promise<void> {
	const filteredHandlers = handlers.map(([emote, callback, button]) => [emote, allFilter, callback, button] as FilteredHandler);
...
async function runHandlers(channel : ChannelID, handlers : FilteredHandler[]) : Promise<void> {
	if (clientReady) {
		await Promise.all(handlers.map(async h => sweepChannel(channel, ...h)));
	}
}
...
async function sweepChannel(channel : ChannelID, emote : EmoteID, filter : Filter, callback : Callback, button : boolean) : Promise<void> {
	...
	await Promise.all(messages.map(async msg => {
			...
			if (button && !(react && react.me)) {
				await msg.react(e);
			}
			if (react) {
				for (let [_, user] of (await react.fetchUsers()).filter(u => !u.equals(client.user))) {
